...
public class ConnectionSettingsConfigurable implements SearchableConfigurable, Configurable.NoScroll {
    ...
    private static final String NAME = "Core Software Delivery Platform";
    ...
    private void testOctaneVersion(ConnectionSettings connectionSettings) {
        ...
        try {
            ...
            if (version.compareTo(OctaneVersion.DYNAMO) < 0) {
                showWarningBalloon("Core Software Delivery Platform version not supported. This plugin works with Core Software Delivery Platform versions starting " + OctaneVersion.DYNAMO.getVersionString());
            }

            if (connectionSettingsView.isSsoAuth()) {
                if (version.compareTo(new OctaneVersion("12.60.14")) < 0) {
                    showWarningBalloon("Login with browser is only supported starting from Core Software Delivery Platform server version: " + OctaneVersion.INTER_P2.getVersionString());
                    connectionSettingsView.setSsoAuth(false);
                }
            }
            ...
            String message = "Failed to determine Core Software Delivery Platform server version, http call to " +
            ...
            SwingUtilities.invokeLater(() -> {
                if (connectionSettingsView != null) {
                    if (ex instanceof OctaneException) {
                        ...
                        connectionSettingsView.setConnectionStatusError("Failed to connect to Core Software Delivery Platform: " + ex.getMessage());
                    }
                }
            });
